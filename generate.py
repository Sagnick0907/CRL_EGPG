...
def prepare_input_sequence(input_text, word_to_index, max_length):
    ...
    indexed_sentence = indexed_sentences[0]  # Assuming a single input sentence
...
def main(dataset_name, model_type):
    ...
    data_folder, model_save_folder, config = load_dataset_config(dataset_name)
    ...
    seq2seq_model, style_extractor_model, _ = load_models(config, model_save_path)
    ...
    generated_sequences = []
    with torch.no_grad():
        for style_vector in style_representations:
    ...
    output_filename = os.path.join(model_save_folder, "generated_paraphrases.txt")
    if os.path.exists(output_filename):
        os.remove(output_filename)
    with open(output_filename, 'a') as output_file:
        for batch in generated_sequences:
            paraphrases = seq2seq_model.getword(idx_to_word, batch)  # Ensure getword is defined in Seq2Seq
